...
class KPFCNN(nn.Module):
    ...
    def forward(self, batch, config):
        ...
        for block_i, block_op in enumerate(self.encoder_blocks):
            ...
            if isinstance(block_op, PyramidBlock):
                ...
                for upi, uplayer in enumerate(block_op.up_layers):
                    ...
                    x_pyr = uplayer.us(x_pyr, batch)
                    skipx = skip_x.pop()
                    print(f"{upi} pyr after ups : x.shape       = {x_pyr.shape}")
                    print(f"{upi} pyr skip poped: skipx.shape   = {skipx.shape}")

                    x_pyr = torch.cat([x_pyr, skipx], dim=1)
                    print(f"{upi} pyr concat    : x.shape       = {x_pyr.shape}")

                    x_pyr = uplayer.ub(x_pyr, batch)
                    print(f"{upi} pyr unary     : x.shape       = {x_pyr.shape}")
        ...
        for block_i, block_op in enumerate(self.decoder_blocks):
            print(
                f"{len(self.decoder_blocks) - block_i}.{str(block_op).split('(')[0][:-1]}:\nin x.shape = {x.shape}")
            if block_i in self.decoder_concats:
                skipx = skip_x.pop()
                print("concat connection")
                print(f"{block_i} ups  input : x.shape = {x.shape}")
                print(f"{block_i} skip poped : skipx.shape = {skipx.shape}")

                x = torch.cat([x, skipx], dim=1)
                print(f"{block_i} concat res : pyt x.shape = {x.shape}")
            x = block_op(x, batch)
            print(f"out x.shape = {x.shape}\n")
        # Head of network
