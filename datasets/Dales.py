...
class DalesDataset(PointCloudDataset):
    ...
    def __init__(self, config, set='training', use_potentials=True, load_data=True):
        ...
        if self.set == 'training':
            self.cloud_names = [f"{self.path}/train_bin/{file}".replace('.ply', '')
                                for file in os.listdir(f'{self.path}/train_bin')]
        elif self.set in ['validation', 'test', 'ERF']:
            self.cloud_names = [f"{self.path}/test_bin/{file}".replace('.ply', '')
                                for file in os.listdir(f'{self.path}/test_bin')]
        else:
        ...
        self.files = [f"{cloud_name}.ply" for cloud_name in self.cloud_names]

        print(f"{set} cloud_names = {self.cloud_names}")
        ...
        if use_potentials:
            ...
            # assert len(self.pot_trees) >= 1
            print(f"Iterating over {len(self.pot_trees)} trees in {set} dataset")
        ...
        while True:
            ...
            with self.worker_lock:
                ...
                try:
                    ...
                    print(f"pot = {self.potentials}")
                    print(f"min_pot = {self.min_potentials}")
                    raise e
    ...
    def load_subsampled_clouds(self):
        ...
        for i, file_path in enumerate(self.files):
            ...
            KDTree_file = join(tree_path, f'{Path(cloud_name).name}_KDTree.pkl')
            sub_ply_file = KDTree_file.replace('_KDTree.pkl', '') + '_sub.ply'
            ...
            if exists(KDTree_file):
                ...
                colors = np.vstack((data['reflectance'], data['reflectance'], data['reflectance'])).T
                ...
                write_ply(sub_ply_file,
                          ...
                          ['x', 'y', 'z', 'reflectance1', 'reflectance2', 'reflectance3', 'class'])
        ...
        if self.use_potentials:
            ...
            for i, file_path in enumerate(self.files):
                ...
                # Name of the input files
                coarse_KDTree_file = join(tree_path, f'{Path(cloud_name).name}_coarse_KDTree.pkl')
        ...
        if self.set in ['validation', 'test']:
            ...
            for i, file_path in enumerate(self.files):
                ...
                proj_file = join(tree_path, f'{Path(cloud_name).name}_proj.pkl')
...
class DalesSampler(Sampler):
    ...
    def calibration(self, dataloader, untouched_ratio=0.9, verbose=False, force_redo=False):
        ...
        if verbose:
            ...
            print(f"batch_lim_dict = {batch_lim_dict}")
        ...
        if redo:
            ...
            if not breaking:
                ...
                plt.savefig('calibration_error1_Dales.png')

                plt.figure()
                plt.plot(debug_b)
                plt.plot(debug_estim_b)
                plt.ylabel("cloud inds length")

                plt.savefig('calibration_error2_Dales.png')

                plt.figure()
                plt.plot(smooth_errors)
                plt.ylabel("smooth error")

                plt.savefig('calibration_error3_smooth_errors_Dales.png')
...
class DalesCustomBatch:
    ...
    @staticmethod
    def shapes_str(arr_list: list) -> str:
        return f"{[arr.shape for arr in arr_list]}"

    def __str__(self) -> str:
        return (
            f"points        = {self.shapes_str(self.points)}\n"
            f"neighbours    = {self.shapes_str(self.neighbors)}\n"
            f"pools         = {self.shapes_str(self.pools)}\n"
            f"upsamples     = {self.shapes_str(self.upsamples)}\n"
            f"lengths       = {self.shapes_str(self.lengths)}\n"
            f"features      = {self.features.shape}\n"
            f"labels        = {self.labels.shape}\n"
            f"scales        = {self.scales.shape}\n"
            f"rots          = {self.rots.shape}\n"
            f"cloud_inds    = {self.cloud_inds.shape}\n"
            f"center_inds   = {self.center_inds.shape}\n"
            f"input_inds    = {self.input_inds.shape}"
        )
